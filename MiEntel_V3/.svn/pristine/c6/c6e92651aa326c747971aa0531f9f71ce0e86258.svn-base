	    ...
	    $.ajax({
			...
			success: function (resp){
		        ...
		        $.each(list, function(itemNo, item) {
		        	...
		        	if (itemNo == 0) {
		        		...
		        		if (arrItem[0] == 'IM' || arrItem[0] == 'IMPP') {
		        			...
		        			url = '<r:pageUrl pageLabel="${productoController.pageLabelBolsas}" />&tab=contratadas';
		        			if(arrItem.length == 1){
		        				txt = arrItem[0];
		        			}
		        		}
            		...
            		if(itemNo != 0 && ((itemNo + 1) % 3) == 0){ 
   						htm += '</ul>';
						// Caso de borde: cantidad total de productos es múltiplo de 3 
	            ...
	            $('#productoscontratados').html(htm);
	          	...
	          	//si solo son 3 no mostrar paginador
	            if(list.length <= 3){ 
	            	$('#paginador-container').hide();
	            } else {
...
<div class="db-tabla">
	...
	<div class="db-tabla-cabecera">
		...
		<div class="db-cabecera-cuerpo">
			<div class="db-titulo db-titulo-productos">Productos contratados</div>
		</div>
	</div>
	...
	<div class="alerta-tabla-reintento" id="alerta-tabla-reintento-productos"></div>
      ...
      <div id="contenido-tabla-productos">
		...
		<div id="paginador-container" class="db-paginador clearfix">
