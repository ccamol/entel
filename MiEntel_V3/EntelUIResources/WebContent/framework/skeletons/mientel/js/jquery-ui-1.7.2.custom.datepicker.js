    ...
    $.extend(Datepicker.prototype, {
        ...
        _showDatepicker: function(input){
            ...
            */
			...
			// INC000003690125
            // se corrige posicionamiento de datepicker, ahora es en base al input
            // se agrega fix para safari, posicionamiento de la tabla del calendar
			var iTop = $(input).offset().top + $(input).height() + 8;
        ...
        _updateDatepicker: function(inst){
			...
			// INC000003690125
            // fix para posicionamiento de la tabla en Safari 4
			if ($.browser.safari && (navigator.appVersion.indexOf('4.') != -1)) {
				$("#jq-dp-table-calendar").css({
					margin: "15px 0 0 -83px"
				});
        ...
        _generateHTML: function(inst){
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                    ...
                    // INC000003690125
                    // se agrega id a la tabla del calendario para poder aplicar regla css que corrige posicionamiento en safari 4
                    //calender += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix' + cornerClass + '">' + (/all|left/.test(cornerClass) && row == 0 ? (isRTL ? next : prev) : "") + (/all|right/.test(cornerClass) && row == 0 ? (isRTL ? prev : next) : "") + this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate, selectedDate, row > 0 || col > 0, monthNames, monthNamesShort) + '</div><table class="ui-datepicker-calendar"><thead><tr>';
                    calender += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix' + cornerClass + '">' + (/all|left/.test(cornerClass) && row == 0 ? (isRTL ? next : prev) : "") + (/all|right/.test(cornerClass) && row == 0 ? (isRTL ? prev : next) : "") + this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate, selectedDate, row > 0 || col > 0, monthNames, monthNamesShort) + '</div><table id="jq-dp-table-calendar" class="ui-datepicker-calendar"><thead><tr>';
            ...
            if (secondary || !changeYear) {
                ...
                if (years.length != 2) {
                    ...
                    if (years[0].charAt(0) == "+" || years[0].charAt(0) == "-") {
                    	// INC000003690125, se aplica fix para bug
                    	// http://bugs.jqueryui.com/ticket/2627
                    	// http://code.google.com/p/jquery-ui/source/detail?r=123
