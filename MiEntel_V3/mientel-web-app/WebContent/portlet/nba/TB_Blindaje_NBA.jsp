...
function rechazarOferta(codOferta){
	var url='<%=request.getContextPath()%>/portlet/nba/rechazoOfertaNBAJson.faces';
	var status;
...
function marcaSmartPixel(){	
	var url='<%=request.getContextPath()%>/portlet/nba/TB_BlindajeNBAJson.faces';
...
$(document).ready(function() {
	$("#frame_blindajeNBA").attr("src", urlNBA);
	...
	else
	{
	   ...
	   if ($.browser.msie && parseInt($.browser.version) == 8) {
		...
		$('#frame_blindajeNBA').load(function(response) {
		   timer = setTimeout("cargarError();", 20000);			
			if (i == 1) 
			{
				if (mostroError == 0) {
					 $('.frame_blindajeNBA').show();
				}
		     }	
			clearTimeout(timer);
			i = i + 1;
		...
		<div class="TB_ajaxContent" allowtransparency="true" >		    	        
				<iframe id="frame_blindajeNBA" allowtransparency="true" src="<h:outputText value="" />"  frameBorder=0 width="701"  height="1315"></iframe>
		</div>
