...
public class RutBean implements Serializable {
	private static final Logger LOGGER = Logger.getLogger(RutBean.class);
    /**
     * 
     */
    ...
    public RutBean(String rutCompleto) {
        ...
        if (numero <= 0) {
            LOGGER.error( new NumberFormatException("rut debe ser positivo: " + numero));
        }

        if (!validate(numero, dv)) {
            LOGGER.error( new IllegalArgumentException(rutCompleto + " is an invalid RUT"));
        }
     ...
     */
    public static void rutToMayus(RutBean rutBean) {
    	if (rutBean.digito == 'k'){
    		rutBean.digito = 'K';
        }
