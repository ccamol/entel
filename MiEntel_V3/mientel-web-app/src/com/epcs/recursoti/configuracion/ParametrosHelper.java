...
import com.epcs.bean.ListadoVasIds;
...
//    private static final String REGIONES_PROPERTY = "region";
    ...
    private static String[] getParametrosAvailableValues(String type) {
        ...
        String availableValues = MiEntelProperties.getProperty("parametros." + type);
    ...
    private static MultitiendaBean getMultitiendaBeanByParametro(
        ...
        try {
            ...
            String nombre = MiEntelProperties.getProperty(keyPrefix + "nombre");
            String id = MiEntelProperties.getProperty(keyPrefix + "id");
            int tipo = Integer.parseInt(MiEntelProperties.getProperty(keyPrefix
                    + "tipo"));
            int digitos = Integer.parseInt(MiEntelProperties
     ...
     */
        ...
        try {
            ...
            String codigo = MiEntelProperties.getProperty(keyPrefix + ".id");
            String descripcion = MiEntelProperties.getProperty(keyPrefix
                    + ".desc");
	...
	public static List<CodeDescBean> getAAABeanList() {
		...
		for (int i = 0; i < availableAAAValue.length; i++) {
			...
			try {
				String codigo = MiEntelProperties.getProperty("aaa."
						+ availableAAAValue[i] + ".code");
				String descripcion = MiEntelProperties.getProperty("aaa."
    ...
    private static String[] getAvailableAAAValues() {
        ...
        String availableValues = MiEntelProperties.getProperty("aaa");
    ...
    public static MontoRecargaBean getMontoRecargaTarjetaCredito(String montoId) {
        ...
        String monto = MiEntelProperties
                .getProperty("parametros.montoRecargaTarjetaCredito." + montoId
                        + ".monto");
        String vigencia = MiEntelProperties
                .getProperty("parametros.montoRecargaTarjetaCredito." + montoId
                        + ".vigencia");

        if (Utils.isEmptyString(monto) || Utils.isEmptyString(vigencia)) {
            return null;
        }

        return new MontoRecargaBean(Double.valueOf(monto), vigencia);
    }

    /**
     * Entrega una lista de {@link MontoRecargaBean} con los montos para
     * recargas de EntelTicket que esten indicados en las propiedades de la
     * aplicacion, bajo el key parametros.montoRecargaTarjetaCredito
     * 
     * @return {@link List} de {@link MontoRecargaBean} con los montos para
     *         recargas de tarjetas de credito
     */
    public static List<MontoRecargaBean> getMontosRecargaEntelTicket() {

        List<MontoRecargaBean> parametrosList = new ArrayList<MontoRecargaBean>();

        String[] availableMontos = getParametrosAvailableValues("montoRecargaEntelTicket");
        ...
        for (int i = 0; i < availableMontos.length; i++) {
            ...
            // si el monto esta mal indicado se ignora, para evitar
            // NullPointerException
            MontoRecargaBean monto = getMontoRecargaEntelTicket(montoId);
            if (monto != null) {
                parametrosList.add(monto);
            }
        }
    ...
    /**
     * Obtiene de los parametros el monto y vigencia de un monto y lo entrega
     * como un {@link MontoRecargaBean}, cuyo codigo es el monto y descripcion
     * es el monto formateao seguido de la vigencia
     * 
     * @param montoId
     *            String id del parametro monto
     * @return {@link MontoRecargaBean} con monto como codigo/descripcion
     */
    public static MontoRecargaBean getMontoRecargaEntelTicket(String montoId) {

        String monto = MiEntelProperties
                .getProperty("parametros.montoRecargaEntelTicket." + montoId
                        + ".monto");
        String vigencia = MiEntelProperties
                .getProperty("parametros.montoRecargaEntelTicket." + montoId
                        + ".vigencia");
    ...
    public static String getNombreMultitienda(String multitienda){
        return MiEntelProperties.getProperty("parametros."+MULTITIENDAS_PROPERTY+"."+multitienda+".nombre");
    }
    ...
    private static List<CodeDescBean> getParamMoneyList(String type) {
        ...
        for(int i = 0; i < parametros.length; i++) {            
            String parametro = parametros[i];            
            parametrosList.add(new CodeDescBean(parametro, "$"+Utils.formatMoney(Double.parseDouble(parametro))));
        }
     ...
     * @return
     */
    private static TipoPlanBean getTipoPlanBeanByParametro(String type,
            String parametro) {

    	TipoPlanBean tipoPlanBean = null;
        try {
            String keyPrefix = "parametros." + type + "." + parametro;
            String codigo = MiEntelProperties.getProperty(keyPrefix + ".id");
            String mercado = MiEntelProperties.getProperty(keyPrefix + ".mercado");
            String nombre = MiEntelProperties.getProperty(keyPrefix + ".nombre");
            String descripcion = MiEntelProperties.getProperty(keyPrefix
                    + ".desc");
            tipoPlanBean = new TipoPlanBean(codigo, mercado, nombre, descripcion);
        } catch (MissingResourceException e) {
            LOGGER.warn("Parametro '" + parametro
                    + "' no encontrado para el tipo '" + type + "'");
        }
     ...
     * @return
     */
    private static List<TipoPlanBean> getTipoPlanParametrosList(String type) {

        List<TipoPlanBean> parametrosList = new ArrayList<TipoPlanBean>();
        ...
        for(int i = 0; i < parametros.length; i++) {
            ...
            String parametro = parametros[i];
            TipoPlanBean tipoPlan = getTipoPlanBeanByParametro(type, parametro);
            if(tipoPlan != null) {
                parametrosList.add(tipoPlan);
            }
        }
    ...
    /**
     * Listado de tipos de planes.
     * @return
     */
    public static List<TipoPlanBean> getListTiposPlan() {
        List<TipoPlanBean> tipoPlanList = getTipoPlanParametrosList(TIPO_PLAN);
        return tipoPlanList;
    }
    

    /**
     * Listado de prefijos Red Fija
     * @return
     */
    public static List<SelectItem> getPrefijoRedFijaParametrosList() {
    	List<SelectItem> listSelect = new ArrayList<SelectItem>();
    	String [] parametros = getParametrosAvailableValues(PREFIJO_REDFIJA);
        SelectItem item;
        for (int i = 0; i < parametros.length; i++) {
     ...
     * @return
     */
    public static List<TipoPlanBean> getListTiposPlanBam() {
        List<TipoPlanBean> tipoPlanList = getTipoPlanParametrosList(TIPO_PLAN_BAM);
        return tipoPlanList;
    }   
    ...
    public static String getRespuestaServicio(String type , String dato)
    {
       ...
       try
    	  { 
    	   respuesta = MiEntelProperties.getProperty("parametros." + type+"."+dato); 
    	   if(respuesta==null)
    	   {  
    		 respuesta = dato;
    ...
    public static String getTipoDeBolsasMasVendidas() {
    	 String tipo="";
    	 try {
    		 tipo =  MiEntelProperties.getProperty(TIPO_BOLSAS_MAS_VENDIDAS);     	   
          } catch (Exception e) {
   ...
   public static CodeDescBean getTipoReclamoFull(String codTipoReclamo,String tipo) {
       ...
       String codigo = MiEntelProperties
               .getProperty("parametros."+tipo+"."+codTipoReclamo
                       + ".codigo");
       String descripcion = MiEntelProperties
               .getProperty("parametros."+tipo+"."+codTipoReclamo
                       + ".descripcion");

       if (Utils.isEmptyString(codigo) || Utils.isEmptyString(descripcion)) {
           return null;
       }
    ...
    * @return
    */
   public static List<FamiliaSuscripcionBean> getSuscripcionFamilia(String type) {

       List<FamiliaSuscripcionBean> parametrosList = null;
       List<CombinacionVas> listCombinacion=null;
       String[] parametros = getParametrosAvailableValues(type);
       ...
       parametrosList=new ArrayList<FamiliaSuscripcionBean>();

       for(int i = 0; i < parametros.length; i++) {
    	   String parametro = parametros[i];
    	   listCombinacion= getVasByFamily(type,parametro);
    	   String[] description = getParametrosAvailableValues(type.trim()+"."+parametro+".description");
    	   if(listCombinacion != null) {
    		   FamiliaSuscripcionBean fb= new FamiliaSuscripcionBean();
    		   fb.setFamilia(parametros[i]);
    		   fb.setListadoVasid(listCombinacion);
    		   fb.setDescription(description);
    		   parametrosList.add(fb);
    	   }
       }
    ...
    * @return
    */
   private static  List<CombinacionVas> getVasByFamily(String type,String parametro) {
	   ArrayList<CombinacionVas> listado=new ArrayList<CombinacionVas>();
	   try {
		   String keyPrefix = type.trim()+"."+parametro;
		   String[] familyvas = getParametrosAvailableValues(keyPrefix+".vasid");
		   // Si no hay parametros se entrega lista vacia
		   if (familyvas == null || familyvas.length == 0) {
			   return null;
		   }  
		   for(int i = 0; i < familyvas.length; i++) {
			   String cadenaSearch=keyPrefix+"."+familyvas[i];
			   String[] listVas = getParametrosAvailableValues(cadenaSearch+".combinacion");
			   if (familyvas == null || familyvas.length == 0) {
				   break;
			   }  
			   for (int j=0;j<listVas.length;j++){
				   CombinacionVas cv=new CombinacionVas();
				   String[] listByCombinacion= getParametrosAvailableValues(cadenaSearch+"."+listVas[j]);
				   if (listByCombinacion == null || listByCombinacion.length == 0) {
					   continue;
				   }  
				   cv.setListadosearch(listByCombinacion);
				   cv.setSearch(listVas[j]);
				   listado.add(cv);
				   cv=null;
			   }
		   }
		   //family = new FamiliaSuscripcionBean(parametro, suscripciones);
